...
class JSONDatabaseServer:
    ...
    def insert_sample_data(self, cursor):
        ...
        try:
            ...
            if cursor.fetchone()[0] == 0:
                ...
                self.logger.info("已插入用户示例数据")
            
            # 插入患者详细信息
            cursor.execute("SELECT COUNT(*) FROM patients")
            if cursor.fetchone()[0] == 0:
                cursor.execute("""
                    INSERT INTO patients (patient_id, name, birth_date, id_card, phone, email) VALUES 
                    (1, '张三', '1990-01-01', '110101199001010001', '13800138000', 'zhangsan@example.com'),
                    (2, '李四', '1985-05-15', '110101198505150002', '13900139000', 'lisi@example.com'),
                    (3, '王五', '1992-03-20', '110101199203200003', '15800158000', 'wangwu@example.com')
                """)
            ...
            if cursor.fetchone()[0] == 0:
                cursor.execute("""
                    INSERT INTO doctors (doctor_id, name, employee_id, department, max_patients, fee, work_schedule, is_available) VALUES 
                    (4, '王医生', 'DOC001', '内科', 30, 50.0, '{"monday": "9:00-17:00", "tuesday": "9:00-17:00", "wednesday": "9:00-17:00", "thursday": "9:00-17:00", "friday": "9:00-17:00"}', 1),
                    (5, '刘医生', 'DOC002', '外科', 25, 80.0, '{"monday": "8:00-18:00", "tuesday": "8:00-18:00", "wednesday": "8:00-18:00", "thursday": "8:00-18:00", "friday": "8:00-18:00", "saturday": "9:00-15:00"}', 1),
                    (6, '陈医生', 'DOC003', '儿科', 40, 60.0, '{"monday": "9:00-17:00", "tuesday": "9:00-17:00", "wednesday": "9:00-17:00", "thursday": "9:00-17:00", "friday": "9:00-17:00"}', 1)
                """)
            ...
            if cursor.fetchone()[0] == 0:
                cursor.execute("""
                    INSERT INTO appointments (patient_id, doctor_id, appointment_time, status, fee_paid, queue_number) VALUES 
            ...
            if cursor.fetchone()[0] == 0:
                cursor.execute("""
                    INSERT INTO medical_records (patient_id, doctor_id, diagnosis, symptoms, visit_time) VALUES 
                    (1, 4, '感冒', '发热、咳嗽、流涕', '2024-02-01 09:30:00'),
                    (2, 5, '阑尾炎', '右下腹痛', '2024-01-28 10:30:00')
                """)
            ...
            if cursor.fetchone()[0] == 0:
                cursor.execute("""
                    INSERT INTO attendance (doctor_id, check_in_time, check_out_time, status) VALUES 
                    (4, '2024-02-01 08:50:00', '2024-02-01 17:10:00', 'present'),
                    (5, '2024-02-01 07:55:00', '2024-02-01 18:05:00', 'present'),
                    (6, '2024-02-01 08:58:00', NULL, 'present')
                """)
    ...
    def login_match(self, data):
        ...
        # 检查参数是否有效
            ...
            print(2)
            # 查询用户记录
            sql_select = f"SELECT password FROM user WHERE user_name = ?"
    ...
    def register_patient(self, data):
        try:
            ...
            user_id = self.hash_password(phone)
            sql_insert_users = f"insert into users(user_id,username,password_hash,role,created_at) values(?,?,?,?,?)"
            sql_insert_patients =f"insert into patients(patient_id,name, birth_date, id_card, phone, email, created_at) values(?,?,?,?,?,?,?)"
            cursor.execute(sql_insert_users,(user_id,phone,password_hash,'patient',time));
            cursor.execute(sql_insert_patients,(user_id,name,birth_date,id_card,phone,email,time));
            conn.commit()
            conn.close()
    ...
    def register_doctor(self, data):
        try:
            ...
            time =datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            user_id = self.hash_password(employee_id)
            sql_insert_users = f"insert into users(user_id,username,password_hash,role,created_at) values(?,?,?,?,?)"
            sql_insert_doctor =f"insert into doctors(doctor_id,name, employee_id, department,max_patients,  created_at) values(?,?,?,?,?,?)"
            cursor.execute(sql_insert_users,(user_id,employee_id,password_hash,'doctor',time));
            cursor.execute(sql_insert_doctor,(user_id,name,employee_id,department,10,time));
            conn.commit()
            conn.close()
            return "chenggongcharu"
        except Exception as e:
